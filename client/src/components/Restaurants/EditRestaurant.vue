    ...
    <div class="l1">
      ...
      <form v-on:submit.prevent="editRestaurant">
        ...
        <div class="form-group">
          ...
          <input type="text" id="phoneNumber" placeholder="062xxxxxxx" pattern="\d{3}\d{3}\d{4}" minlength="10" maxlength="10" v-model="restaurant.phoneNumber" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="text" id="menuType" placeholder="ประเภทของอาหารที่อยู่ในร้าน" v-model="restaurant.menuType" required>
        </div>
        <div class="form-group">
...
.edit-restaurant-container {
  ...
  align-items: flex-start; /* ตั้งค่าให้เริ่มจากด้านบนของ .l1 และ .l2 */
}

.l1 {
  /* สไตล์สำหรับ .l1 (ฝั่งซ้าย) */
  width: 48%; /* กำหนดความกว้างของ .l1 ให้เล็กน้อยกว่าครึ่งหน้า */
  padding: 10px; /* กำหนดการเว้นระหว่างขอบใน .l1 */
  box-sizing: border-box; /* ไม่ให้ padding มีผลกับขนาดของ .l1 */
  
}

.l2 {
  /* สไตล์สำหรับ .l2 (ฝั่งขวา) */
  width: 40%; /* กำหนดความกว้างของ .l2 ให้เล็กน้อยกว่าครึ่งหน้า */
  padding: 10px; /* กำหนดการเว้นระหว่างขอบใน .l2 */
  box-sizing: border-box; /* ไม่ให้ padding มีผลกับขนาดของ .l2 */
}
...
input {
  width: 100%;
  padding: 5px; /* ปรับความหนาขอบของกล่อง input */
  height: 30px; /* ปรับความสูงของกล่อง input */
  border: 1px solid #ccc;
  border-radius: 5px;
}
